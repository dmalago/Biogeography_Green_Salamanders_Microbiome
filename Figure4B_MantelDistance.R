...
mantel_correlog_jaccard<-mantel.correlog(jaccard_matrix,dist_matrix,nperm=9999, break.pts=c(0,30,temp$break.pts[2:length(temp$break.pts)]))
plot(mantel_correlog_jaccard, alpha=0.05)

#You can also tell the program to go to all the distances, even if you have small numbers of replicates at those distances
temp<-mantel.correlog(bray_matrix,dist_matrix,nperm=9999)
mantel_correlog_bray<-mantel.correlog(bray_matrix,dist_matrix,nperm=9999, break.pts=c(0,30,temp$break.pts[2:length(temp$break.pts)]))
plot(mantel_correlog_bray, alpha=0.05)

#You can also tell the program to go to all the distances, even if you have small numbers of replicates at those distances
temp<-mantel.correlog(unifrac_matrix,dist_matrix,nperm=9999)
mantel_correlog_unifrac<-mantel.correlog(unifrac_matrix,dist_matrix,nperm=9999, break.pts=c(0,30,temp$break.pts[2:length(temp$break.pts)]))
plot(mantel_correlog_unifrac$mantel.res, alpha=0.05,cex.lab=2)

#You can also tell the program to go to all the distances, even if you have small numbers of replicates at those distances
temp<-mantel.correlog(wunifrac_matrix,dist_matrix,nperm=9999)
mantel_correlog_wunifrac<-mantel.correlog(wunifrac_matrix,dist_matrix,nperm=9999, break.pts=c(0,30,temp$break.pts[2:length(temp$break.pts)]))
